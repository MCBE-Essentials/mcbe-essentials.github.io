...
function createTier(disableName) {
	var index =
		document.getElementById('list').children.length == 0
			? 1
			: document.getElementById('list').children.length + 1
	var tierName =
		index - 1 <= tierNames.length - 1
			? tierNames[index - 1]
...
function addTrade(element, tradeData) {
	...
	tradeHTML +=
		'<mcitem style="height:40px; width:40px;" ' +
		(hasEnchants(wants0) ? 'class="enchanted"' : '') +
		' identifier="' +
		wants0.item +
		'" count="' +
		wants0.quantity +
	...
	if (wants1) {
		tradeHTML +=
			'<mcitem style="height:40px; width:40px;" ' +
			(hasEnchants(wants1) ? 'class="enchanted"' : '') +
			' identifier="' +
			wants1.item +
			'" count="' +
			wants1.quantity +
		...
		tradeHTML +=
			'<mcitem style="height:40px;width:40px;visibility:hidden;" identifier="minecraft:air" count="1"></mcitem>'
	}
	tradeHTML +=
		'<img class="trade-arrow" src="https://github.com/bedrock-dot-dev/packs/raw/master/stable/resource/textures/ui/arrow_dark_right.png">'
	tradeHTML +=
		'<mcitem style="height:40px; width:40px;" ' +
		(hasEnchants(gives) ? 'class="enchanted"' : '') +
		' identifier="' +
		gives.item +
		'" count="' +
		gives.quantity +
...
function correctImportedTable(table) {
	...
	for (var i = 0; i < tiers.length; i++) {
		...
		for (var a = 0; a < groups.length; a++) {
			...
			for (var b = 0; b < trades.length; b++) {
				...
				if (!wants[0] || !gives[0]) {
					alert(
						'Your trade table is invalid: missing basic required wants/gives items.'
					)
					location.reload()
				}
				...
				if (wants[0].item.split(':').length != 2) {
					if (wants[0].item.split(':').length == 1) {
						...
						wants[0].item =
							wants[0].item.split(':')[0] +
							':' +
							wants[0].item.split(':')[1]
					}
				}
				...
				if (gives[0].item.split(':').length != 2) {
					if (gives[0].item.split(':').length == 1) {
						...
						gives[0].item =
							gives[0].item.split(':')[0] +
							':' +
							gives[0].item.split(':')[1]
					}
				}
			}
		}
	...
	if (choiceMessage)
		alert(
			'It seems that one or more trades in your trade table use the [choice] feature for either the wants[0], wants[1] or gives item. The choice feature is not supported by this program. The first item in the list has been selected to permanently apply instead. '
		)
