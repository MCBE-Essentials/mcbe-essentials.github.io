...
async function getData(){
  ...
  let tiles = await fetch('https://raw.githubusercontent.com/joaololpvp/mcbe-essentials.github.io/patch-1/data/tile-entities-v2.json').then(response => {return response.json()})
  ...
  let general_data = await fetch('https://raw.githubusercontent.com/joaololpvp/mcbe-essentials.github.io/patch-1/data/general.json').then(response => {return response.json()})
...
if (tags.Trim || tags.customColor || isDyeable || isTrimmable) {
  ...
  if (tags.Trim || isTrimmable) {
    ...
    // Se a tag existir, carrega os valores reais
    if (tags.Trim) {
      ...
      // Se não tiver a tag (é um item novo ou limpo), define o padrão como "none"
    ...
    // 3. Grupo da Cor: Só aparece se tiver a tag de cor OU se for couro
    if (tags.customColor || isDyeable) {
      document.getElementById("item-armor-leathergroup").style.display = "table-cell";
      
    // Referências para as duas áreas (botão e controles)
    ...
    if (tags.customColor) {
        // Se a tag existe: Mostra o input e o botão de remover
        addContainer.style.display = "none";
        controlsContainer.style.display = "block";
        document.getElementById("item-armor-customcolor").value = datHandler.argb.intToHex(tags.customColor.value, true);
    } else {
        // Se é de couro mas NÃO tem a tag: Mostra apenas o botão de adicionar
        addContainer.style.display = "block";
        controlsContainer.style.display = "none";
        // Resetamos o valor do input para branco internamente (caso o usuário adicione depois)
    ...
    // Esconde a aba se o item não for nada relacionado a armadura/cor
...
case 'crafter': {
  ...
  // 2. Preparar a estrutura de itens para o HTML
  if (!tileEntity.Items) {
  ...
  // Garantir que existem 9 itens na lista, um para cada slot (0-8)
  for (let i = 0; i < 9; i++) {
  ...
  // Ordenar a array pelo número do Slot para o índice [0] ser o Slot 0
...
case 'shelfblock': {
    // 1. Se o shelf estiver totalmente vazio (como o seu índice 2), a tag Items não existe. Criamos ela.
    if (!tileEntity.Items) {
        tileEntity.Items = { type: 'list', value: { type: 'compound', value: [] } };
    }
    ...
    // 2. Garante que o array tenha exatamente 3 slots para os <td> do seu HTML
    while (itemsArray.length < 3) {
        itemsArray.push({
            Name: { type: 'string', value: "" }, // O Bedrock usa string vazia para o vazio
            Count: { type: 'byte', value: 0 },
            Damage: { type: 'short', value: 0 },
...
case 'chiseledbookshelf': {
    // 1. Localiza a tabela de slots no seu HTML
    const shelfTable = document.getElementById("tentity-chiseledbookshelf-slot");
    
    // 1.1 LIMPA A TABELA (Isso evita que os itens fiquem se multiplicando visualmente)
    if (shelfTable) {
        // Limpa todas as células (td) da tabela
    ...
    // 2. Garante que a estrutura de Items existe exatamente como no seu NBT
    ...
    // 3. Pega a referência direta dos itens
    let bookshelfItems = tileEntity.Items.value.value;

    // 4. Renderiza os slots (agora na tabela limpa)
    fillItemSlots(bookshelfItems, shelfTable);
    
    // 5. Inicializa os ícones do Bedrock
    if (typeof mcitems !== 'undefined') mcitems.init();
    
    break;
}
case 'vault': {
    // Limpa o slot da chave
    document.getElementById("tentity-vault-key-item").innerHTML = ""; 

    // Referência para facilitar o acesso baseado no seu JSON
    let vConfig = tileEntity.config.value;
    let vStateData = tileEntity.data.value; // Acessa a aba 'data' do Vault

    // Preenche os inputs de texto e números
    ...
    // Lógica do Item: Exatamente como o Decorated Pot
    if(vConfig.key_item && vConfig.key_item.value){
        // Usamos createItemElement para gerar o ícone visual
        document.getElementById("tentity-vault-key-item").appendChild(createItemElement(vConfig.key_item.value));
    }
    
    mcitems.init(); // Atualiza as texturas na tela

    // LÓGICA DO CONTADOR
    let playerCount = 0;
    try {
        // No Bedrock, acessamos a lista -> value (array) -> length
    ...
    // Ajuste no botão de Reset para também zerar o número na tela na hora
    // Ajuste no botão de Reset
document.getElementById("tentity-vault-reset-players").onclick = function() {
    // 1. Pegamos a lista atual
    let currentList = vStateData.rewarded_players.value.value;

    // 2. Verificamos se ela já está vazia
    if (!currentList || currentList.length === 0) {
        if(typeof snackbar === "function") {
            snackbar("The rewarded players list is already empty!");
        }
        return; // Interrompe a função aqui para não abrir o confirm
    }

    // 3. Se não estiver vazia, prossegue com o confirm e o reset
...
case 'tspawner': {
    ...
    window.addTrialMobRow = function(modeSuffix, id = "", weight = 1, equip = "", drops = {}) {
        ...
        // Dados de drop para preenchimento (se existirem)
        const d = {
    ...
    // --- 3. CARREGAMENTO DE CAMPOS GLOBAIS ---
    ...
    // --- 4. DISTRIBUIÇÃO NORMAL VS OMINOUS ---
    ...
    ['normal_config', 'ominous_config'].forEach(mKey => {
        ...
        // Leitura de Mobs e Drops (Hierarquia Bedrock)
...
case 'vault': {
        // Seguindo a estrutura confirmada: currentValidTile.data.config.value
        let vConfig = currentValidTile.data.config.value;

        // Salva as Strings das Loot Tables
        ...
        // Salva os alcances como Float (essencial para o Minecraft reconhecer)
        ...
        // O item e os jogadores já são salvos via referência no objeto direto.
    
    break;
}
case 'tspawner': {
    // 1. Required Player Range (Raiz)
    tileEntity.required_player_range = { 
        type: "int", 
        value: parseInt(document.getElementById("tentity-tspawner-player-range").value) || 14 
    };

    // 2. Função interna para processar as listas seguindo o seu padrão de objetos
    const getListFromUI = (containerId, isMob) => {
        ...
        for (let row of rows) {
            if (isMob) {
                ...
                // Estrutura exigida pelo NBT: data -> entity -> id
                let mobCompound = {
                    data: {
                        type: "compound",
                        value: {
                ...
                // Lógica de Equipamento e Drop Chances
                if (equip.trim() !== "") {
                    ...
                    // Coleta valores de drop (converte para float)
                    ...
                    // Só adiciona slot_drop_chances se houver pelo menos um valor
    ...
    // 3. Salvar Configurações (Normal e Ominous)
    ['normal', 'ominous'].forEach(mode => {
        ...
        // Itens de drop ominous ficam dentro da config normal no seu NBT
        if (mode === 'normal') {
    ...
    // 4. Sincronizar SpawnData (Visual do bloco) com o primeiro mob da lista Normal
    // --- 4. Sincronizar SpawnData (Visual do bloco) ---
    // Segue o padrão do seu NBT: TypeId e Weight (com maiúsculas)
    ...
    if (normalMobRows.length > 0) {
        ...
        // Se o primeiro mob tiver equipamento, sincroniza também no visual
        if (firstEquip.trim() !== "") {
            ...
            // Coleta os drops do primeiro mob para o SpawnData
            const getD = (sel) => {
