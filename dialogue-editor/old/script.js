window.location.href = "/dialogue-editor/";
...
function loadScene(num){
  ...
  var buttons = document.getElementById("buttons").getElementsByClassName("dialogue-button");
...
function updateScene(){
  ...
  for(var i = 0; i < buttons.length; i++){
    if(buttons[i].getAttribute("undef") != "true"){
      ...
      if(buttons[i-1]){
        if(buttons[i-1].getAttribute("undef") == "true"){
...
function editButton(el, index){
  ...
  if(el.parentNode.previousElementSibling.children[0]){
    if(el.parentNode.previousElementSibling.children[0].getAttribute("undef") == "true"){
      alert("You must define all previous buttons before you can define this one.");
      return;
    }
  }
  
  if(el.parentNode.nextElementSibling.children[0]){
    if(el.getAttribute("undef") == "true" || el.parentNode.nextElementSibling.children[0].getAttribute("undef") == "false"){
...
function closeButtonEditor(ignoreUpdating){
  ...
  if(!ignoreUpdating){
    ...
    document.getElementById("buttons").getElementsByClassName("dialogue-button")[buttonIndex].innerHTML = atitle;
    updateScene();
  }
}
...
function deleteButton(){
  ...
  document.getElementById("buttons").getElementsByClassName("dialogue-button")[buttonIndex].setAttribute("undef", "true");
